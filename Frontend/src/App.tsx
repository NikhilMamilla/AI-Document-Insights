...
const App: React.FC = () => {
	...
	return (
		<div className={`${currentDocument ? 'min-h-screen overflow-y-auto' : 'min-h-screen'} bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100`}>
			<header className="bg-white/80 backdrop-blur-sm border-b border-white/20 sticky top-0 z-50">
				<div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 sm:py-4">
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
						...
						<nav className="flex flex-col sm:flex-row sm:space-x-1 space-y-2 sm:space-y-0 bg-gray-100 p-1 rounded-xl w-full sm:w-auto">
							<button onClick={() => setActiveTab('upload')} className={`w-full sm:w-auto px-6 py-2 rounded-lg font-medium transition-all duration-200 ${activeTab === 'upload' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
							...
							<button onClick={() => setActiveTab('history')} className={`w-full sm:w-auto px-6 py-2 rounded-lg font-medium transition-all duration-200 ${activeTab === 'history' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
								<div className="flex items-center space-x-2">
									<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				{activeTab === 'upload' ? (
					<div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
						<div className="space-y-6">
							<div className="bg-white/60 backdrop-blur-sm rounded-2xl border border-white/20 p-4 sm:p-6 shadow-xl">
						...
						<div className="space-y-6">
							{currentDocument ? (
								...
								<div className="bg-white/40 backdrop-blur-sm rounded-2xl border border-white/20 p-8 sm:p-12 text-center">
